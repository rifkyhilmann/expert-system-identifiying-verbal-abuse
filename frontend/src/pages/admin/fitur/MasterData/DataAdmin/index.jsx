...
export default function DataAdmin() {
    ...
    useEffect(() => {
        const result = data.filter(item =>
            item.nm_admin.toLowerCase().includes(search.toLowerCase()) 
        );
        setFilteredData(result);
    }, [search, data]);

    const btnDeleteClick = (id_admin) => {
        const fetchDelete = async () => {
            try {
                const response = await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/admin/${id_admin}`);
    ...
    const columns = [
        ...
        {
            name: 'Nama',
            selector: (row) => row.nm_admin, // Adjust field name
            sortable: true,
        },
        {
            name: 'Username',
        ...
        },
        {
            ...
            cell: (row) => (
                <div className='flex gap-2'>
                    <Link to={`/admin/data-admin/update/${row.id_admin}`}>
                    ...
                    <button 
                        ...
                        onClick={() => btnDeleteClick(row.id_admin)}
                    >
    ...
    return (
        <div className='flex flex-col w-full h-max gap-5'>
            <div className='flex flex-col ml-6'>
                ...
                <p className='text-xs'>Data Admin</p>
            </div>
            <div className='ml-6 mr-6 h-max'>
                <div className='flex items-center justify-between mb-3'>
                    ...
                    />
                    <Link to={'/admin/data-admin/tambah'}>
                        <div className='h-10 w-52 flex items-center justify-center text-primary hover:text-white bg-transparent hover:bg-primary gap-2 rounded cursor-pointer'>
                            ...
                            <p className='text-sm'>Tambah Admin</p>
                        </div>
                    </Link>
                </div>
